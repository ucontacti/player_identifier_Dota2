...
goal_steam_id = 76561197974830099
# goal_hero_id = ""

X.loc[X["Steam_id"] == goal_steam_id, "Label"] = 1
X.loc[X["Steam_id"] != goal_steam_id, "Label"] = 0

y = X["Label"]
X.drop("Label", axis =1 , inplace=True)
X.drop("Hero", axis =1 , inplace=True)
X.drop("Steam_id", axis =1 , inplace=True)
...
result_rm=cross_validate(clf, X, y, cv=5,scoring=['precision', 'recall', 'accuracy', 'f1'])
...
result_rm=cross_validate(clf, X, y, cv=5,scoring=['precision', 'recall', 'accuracy', 'f1'])
print('----------------------The cross validated accuracy score for Logistic Regression is:',round(result_rm["test_accuracy"].mean()*100,2))
