...
def train(model, model_name, steamer, X):
    ...
    for player in steamer.index:
        ...
        result_rm=cross_validate(model, new_X, y, cv=5,scoring={'precision': 'precision', 'recall': 'recall', 'accuracy': 'accuracy', 'f1': 'f1', 'roc_auc': 'roc_auc', 'eer': make_scorer(calculate_eer)}, return_estimator=True)
        ...
        counter += 1
        # print(np.mean([imp.coef_[0] for imp in result_rm["estimator"]], axis=0))
