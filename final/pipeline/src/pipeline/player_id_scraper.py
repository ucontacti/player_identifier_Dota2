...
PLAYER_INDEX_PATH = "../resources/player_index.txt"


def player_link(my_url, webdriver_path):
    options = Options()
...
def scrape(chromedriver_path, num_player): # TODO: num_player limit
    ...
    if exists(PLAYER_INDEX_PATH):
        with open(PLAYER_INDEX_PATH, 'r') as filehandle:
    ...
    for hero_link in DOTABUFF_HEROES:
        ...
        if (len(new_players) > num_player):
            break
        print("Done!")
    new_players = list(set(new_players))
    with open(PLAYER_INDEX_PATH, 'a') as filehandle:
        for listitem in new_players[:num_player]:
            filehandle.write('%s\n' % listitem)